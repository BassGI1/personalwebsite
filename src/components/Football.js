...
export default function Football() {
	...
	useEffect(() => {
		const x = {
			...
			x: 0,
			y: 0,
			topGoalCoords: document
				.getElementById("top-goal")
				.getBoundingClientRect(),
			bottomGoalCoords: document
				.getElementById("bottom-goal")
				.getBoundingClientRect(),
		}
		ballData.current = x
	}, [])
	...
	const animateBall = (e) => {
		if (!ballMoving && !golaso) {
			...
			if (possX < -(ballData.current.fieldDimensions.width / 2))
				ballData.current.x =
					-(ballData.current.fieldDimensions.width / 2) + 8
			...
			if (possY < -(ballData.current.fieldDimensions.height / 2))
				ballData.current.y =
					-(ballData.current.fieldDimensions.height / 2) + 8
			else if (possY > ballData.current.fieldDimensions.height / 2)
				ballData.current.y =
					ballData.current.fieldDimensions.height / 2 - 8
			else ballData.current.y = possY

			ballData.current.ball.style.transform = `translate(${
				ballData.current.x
			}px, ${ballData.current.y}px) rotate(${Math.random() * 3}turn)`
		}
	}
	...
	const checkGoal = (e) => {
		if (ballData.current.x >= -10 && ballData.current.x <= 10) {
			if (ballData.current.y <= -155) {
				ballData.current.ball.style.transform =
					"translate(0px, 0px)"
